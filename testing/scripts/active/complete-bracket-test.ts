 ...
 * 2. Populate data with formulas (populate-test-sheet --skip-region-formatting) - Creates tabs first!
 * 3. Apply ALL formatting from extracted rules (apply-formatting-standalone.ts) - 1995 cells, 460 borders, 16 merges
...
try {
  ...
  console.log('\nStep 2/4: Populating data with formulas...');
  console.log('  Creates tabs first, then adds data (using --skip-region-formatting)\n');
  execSync(`npm run populate-test-sheet ${SHEET_ID} -- --skip-region-formatting`, {
    stdio: 'inherit',
    cwd: path.join(__dirname, '../../..')
  });

  console.log('\nStep 3/4: Applying formatting from codified rules...');
