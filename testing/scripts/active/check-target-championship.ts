...
async function checkChampionshipFormatting() {
  ...
  console.log('Checking championship cell formatting in TARGET sheet...\n');

  // Read O17 (row 16, col 14) with formatting
  ...
  const cell = rowData[0]?.values?.[0]; // O17

  if (!cell) {
    console.log('❌ Championship cell O17 not found');
    return;
  }

  const format = cell.effectiveFormat || cell.userEnteredFormat;

  console.log('Championship Cell O17 Formatting:');
  ...
  console.log('\n  Text Color:', format?.textFormat?.foregroundColor);
  ...
  console.log('\n  Font Size:', format?.textFormat?.fontSize);
...
checkChampionshipFormatting().catch(console.error);
